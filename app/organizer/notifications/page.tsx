  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (data.user?.role !== 'organizer') { router.push('/login'); return; }
        setUser(data.user);
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen bg-green-50">
        {user && <OrganizerSidebar user={user} />}
        {user && <OrganizerNav user={user} />}
          ...
          <div className="max-w-2xl mx-auto">
            ...
            {notifications.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-6 py-16 text-center">
                ...
                <p className="text-xs text-gray-400">Здесь появятся уведомления об откликах, статусах и истечении сроков проектов</p>
              </div>
            ) : (
