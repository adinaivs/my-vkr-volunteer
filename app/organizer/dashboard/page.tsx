      ...
      <div className="min-h-screen bg-green-50">
        ...
        <DynamicContent>
          ...
          <div className="mb-4 sm:mb-8">
            ...
            <p className="hidden sm:block text-gray-600">{t.dashboard?.subtitle || 'Вот обзор ваших проектов и активности'}</p>
          </div>

          {/* Stats Grid */}
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 sm:gap-4 mb-4 sm:mb-8">
            <div className="bg-gradient-to-br from-[#00CC00] to-emerald-500 rounded-xl p-3 sm:p-4 shadow-lg text-white col-span-1">
              <div className="flex flex-col gap-1">
                ...
                <div className="text-xl sm:text-2xl font-bold">{user.organizerProfile?.freePostsRemaining ?? 0}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-6 mb-4 sm:mb-8">
            ...
            <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl border border-gray-300">
              ...
              {stats && stats.totalProjects > 0 ? (
                <div className="space-y-5">
                  ...
                  <SummaryRow
                    ...
                    color="bg-blue-400"
                  />
                  <div className="pt-3 sm:pt-4 border-t border-gray-100 grid grid-cols-2 gap-2 sm:gap-4">
                    <div className="text-center">
                      <div className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.totalVolunteers}</div>
                      <div className="text-xs text-gray-500 mt-1">{t.dashboard?.allVolunteers || 'Всего волонтёров участвовало'}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl sm:text-3xl font-bold text-orange-500">{stats.pendingApplications}</div>
          ...
          {/* Recent Applications */}
          <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl border border-gray-300 mb-4 sm:mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
            <Link
              href="/organizer/projects"
              className="bg-gradient-to-br from-[#00CC00] to-emerald-500 rounded-2xl p-4 sm:p-6 text-white hover:shadow-lg transition-shadow group"
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 rounded-xl flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 transition-transform">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
              </div>
              <h4 className="text-base sm:text-lg font-bold mb-1">{t.dashboard?.createProject || 'Создать проект'}</h4>
              <p className="text-xs sm:text-sm text-emerald-50">{t.dashboard?.createProjectDesc || 'Опубликуйте новый волонтёрский проект'}</p>
            </Link>

            <Link
              href="/organizer/volunteers"
              className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl border border-gray-300 hover:shadow-2xl transition-shadow group"
            >
              ...
              <h4 className="text-base sm:text-lg font-bold text-gray-900 mb-1">{t.dashboard?.volunteersTitle || 'Волонтёры'}</h4>
              <p className="text-xs sm:text-sm text-gray-600">{t.dashboard?.volunteersDesc || 'Просмотр заявок и волонтёров'}</p>
            </Link>
            ...
            >
              ...
              <h4 className="text-base sm:text-lg font-bold text-gray-900 mb-1">{t.dashboard?.reportsTitle || 'Отчёты'}</h4>
              <p className="text-xs sm:text-sm text-gray-600">{t.dashboard?.reportsDesc || 'Скачать отчёты по проектам'}</p>
            </Link>
          </div>
        </DynamicContent>
...
function StatCard({
  ...
  return (
    <div className={`bg-white rounded-xl p-3 sm:p-4 shadow-xl border ${highlight ? 'border-orange-300' : 'border-gray-300'}`}>
      <div className="flex items-center gap-2 sm:gap-3">
        <div className={`w-8 h-8 sm:w-9 sm:h-9 ${iconBg} rounded-lg flex items-center justify-center flex-shrink-0`}>
          <svg className={`w-4 h-4 sm:w-5 sm:h-5 ${iconColor}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={iconPath} />
          </svg>
        </div>
        <div>
          <div className={`text-xl sm:text-2xl font-bold ${highlight ? 'text-orange-500' : 'text-gray-900'}`}>{value}</div>
