...
export default function OrganizerChatsPage() {
  ...
  const allChats = [
    ...directChats.map(chat => ({
      ...
      route: `/organizer/chats/direct-${chat.id}`,
              ...
              <div className="flex-1 overflow-y-auto">
                {totalChats === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full p-6 text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-1">{t.chats?.noChatsHint || 'Создайте проект или напишите волонтёру'}</p>
                  </div>
                ) : (
                  allChats.map((chat) => (
                    <button
