...
function formatDate(iso: string, locale: string = 'ru') {
  const localeStr = locale === 'kg' ? 'ky-KG' : 'ru-RU';
  return new Date(iso).toLocaleDateString(localeStr, { day: 'numeric', month: 'short', year: 'numeric' });
}
...
export default function VolunteerDashboard() {
  ...
  // Подбор формы числительного для заявок (RU склонение / в KG одна форма)
  const pluralizePending = (n: number): string => {
    if (locale === 'kg') {
      return t.dashboard?.pendingApplications || 'арыз каралууда';
    }
    const mod10 = n % 10;
    const mod100 = n % 100;
    if (mod10 === 1 && mod100 !== 11) {
      return t.dashboard?.pendingApplication || 'заявка на рассмотрении';
    }
    if (mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14)) {
      return t.dashboard?.pendingApplicationsFew || 'заявки на рассмотрении';
    }
    return t.dashboard?.pendingApplications || 'заявок на рассмотрении';
  };

  // Локализованные статусы проекта
  const getStatusInfo = (status: string): { label: string; cls: string } => {
    const label = t.status?.[status] || status;
    return { label, cls: PROJECT_STATUS_STYLES[status] ?? 'bg-gray-100 text-gray-600' };
  };
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen bg-green-50">
        ...
        <DynamicContent>
          {/* Hero */}
          <section className="bg-gradient-to-r from-[#00CC00] to-emerald-500 rounded-2xl p-5 sm:p-8 md:p-10 text-white mb-6 sm:mb-8 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
              <div>
                ...
                {stats && stats.pendingApplications > 0 && (
                  <div className="mt-3 inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-3 py-1.5 text-sm">
                    ...
                    {stats.pendingApplications} {pluralizePending(stats.pendingApplications)}
                  </div>
                )}
              </div>
              <div className="flex gap-2 sm:gap-3 flex-wrap sm:flex-nowrap">
                <Link
                  ...
                  className="px-4 sm:px-5 py-2 sm:py-2.5 bg-white text-[#00CC00] rounded-full text-sm font-semibold hover:bg-emerald-50 transition-colors whitespace-nowrap"
                >
                  {t.common?.findProject || 'Найти проект'}
                </Link>
                <Link
                  href="/volunteer/my-projects"
                  className="px-4 sm:px-5 py-2 sm:py-2.5 bg-white/20 backdrop-blur-sm text-white rounded-full text-sm font-semibold hover:bg-white/30 transition-colors whitespace-nowrap border border-white/30"
                >
          ...
          {stats && stats.ratingCount > 0 && (
            <div className="bg-white rounded-2xl p-4 sm:p-5 shadow-sm border border-gray-100 mb-6 sm:mb-8 flex flex-wrap sm:flex-nowrap items-center gap-4 sm:gap-5">
              <div className="text-center shrink-0">
                <div className="text-3xl sm:text-4xl font-bold text-gray-900">{stats.trustScore.toFixed(1)}</div>
                <div className="text-xs text-gray-500 mt-0.5">{t.dashboard?.outOfFive || 'из 5.0'}</div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-0.5 sm:gap-1 mb-1 flex-wrap">
                  {[1, 2, 3, 4, 5].map(s => (
                    <svg key={s} className={`w-4 h-4 sm:w-5 sm:h-5 ${stats.trustScore >= s ? 'text-yellow-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                  <span className="text-xs sm:text-sm text-gray-500 ml-1">{stats.ratingCount} {t.dashboard?.ratings || 'оценок'}</span>
                </div>
                <p className="text-xs sm:text-sm text-gray-500">{t.dashboard?.reliabilityRating || 'Ваш рейтинг надёжности — формируется по оценкам организаторов'}</p>
              </div>
                        ...
                        {pct !== null && (
                          <div>
                            <div className="flex justify-between text-xs text-gray-500 mb-1">
                              <span>{t.dashboard?.tasksLabel || t.common?.tasks || 'Задачи'}</span>
                        ...
                        {spotsLeft <= 5 && spotsLeft > 0 && (
                          <div className="absolute top-3 right-3">
                            <span className="px-2 py-1 bg-orange-500 rounded-full text-xs font-medium text-white">
                              {t.dashboard?.spotsLeft || 'Осталось'} {spotsLeft}
                            </span>
                          </div>
                        )}
                      </div>
                      <div className="p-4 flex flex-col flex-1">
                        ...
                        <div className="space-y-1.5 mb-4">
                          ...
                          <div className="flex items-center gap-1.5 text-xs text-gray-500">
                            ...
                            {project.currentVolunteers} / {project.maxVolunteers} {t.common?.volunteers || 'волонтёров'}
                          </div>
                        </div>
                        <Link
                        ...
                        >
                          {t.dashboard?.moreDetails || t.common?.moreDetails || 'Подробнее'}
                        </Link>
                      </div>
                    </div>
                  );
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-gray-100 flex items-center gap-2 sm:gap-3">
