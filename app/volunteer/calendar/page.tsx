...
const COLORS = [
  '#00CC00','#2563eb','#dc2626','#d97706','#7c3aed',
  '#db2777','#0891b2','#059669','#64748b',
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 z-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-gray-900">{t.calendar?.addEvent || 'Новое событие'}</h2>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1">{t.calendar?.eventTitle || 'Название'} *</label>
            <input
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1">{t.calendar?.eventDescription || 'Описание'}</label>
            <textarea
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">{t.calendar?.startTime || 'Начало'}</label>
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">{t.calendar?.endTime || 'Конец'}</label>
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="allday" className="text-sm text-gray-700">{t.calendar?.allDay || 'Весь день'}</label>
          </div>
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1">{t.calendar?.eventLocation || 'Место'}</label>
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-2">{t.calendar?.color || 'Цвет'}</label>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button onClick={handleSave} disabled={saving}
              ...
              {saving ? (t.common?.loading || 'Сохранение...') : (t.calendar?.save || 'Сохранить')}
            </button>
          </div>
        </div>
      </div>
...
function DayDrawer({ date, events, onClose, onDelete, addedTaskIds, onAddTask }: DayDrawerProps) {
        ...
        <div className="overflow-y-auto flex-1 p-5">
          {events.length === 0 ? (
            ...
            <div className="space-y-3">
              {events.map(ev => (
                <div key={ev.id} className="flex gap-3 p-4 rounded-xl border border-gray-100 hover:border-gray-200 bg-white shadow-sm">
                  <div className="w-1 rounded-full flex-shrink-0 mt-0.5" style={{ backgroundColor: ev.color, minHeight: '40px' }} />
                  <div className="flex-1 min-w-0">
                    ...
                    {ev.type === 'task' && (
                      <div className="mt-1 space-y-1">
                        ...
                        <div className="text-xs text-gray-500">Проект: {ev.project.title}</div>
                        {ev.skill && <div className="text-xs text-gray-400">Навык: {ev.skill.name}</div>}
                        {ev.description && <div className="text-xs text-gray-500 line-clamp-2 mt-1">{ev.description}</div>}
                      </div>
                    )}
                    {ev.type === 'personal' && (
                      <div className="mt-1 space-y-1">
                        ...
                        {ev.linkedTask && (
                          <div className="text-xs text-gray-400">Задача: {ev.linkedTask.title}</div>
                        )}
                      </div>
                    )}
                  </div>
...
function YearView({ year, events, today, onSelectDay }: {
  year: number;
  events: CalendarEvent[];
  today: string;
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      {Array.from({ length: 12 }, (_, mi) => {
        ...
        return (
          <div key={mi} className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm hover:shadow-md transition-shadow">
            <div className="text-center font-semibold text-gray-800 text-sm mb-2">{MONTHS_RU[mi]}</div>
...
function MonthView({ year, month, events, today, onSelectDay }: {
  year: number;
  month: number;
  events: CalendarEvent[];
  today: string;
  ...
  while (cells.length < 42) {
    ...
    cells.push({ date: `${y}-${String(m).padStart(2, '0')}-${String(idx).padStart(2, '0')}`, current: false });
  }
        ...
        {cells.map(({ date, current }) => {
          ...
          return (
            <div key={date}
              ...
              className={`min-h-[100px] p-1.5 cursor-pointer hover:bg-gray-50 transition-colors ${!current ? 'bg-gray-50/50' : ''}`}>
              ...
              <div className="space-y-0.5">
                {shown.map(ev => (
                  <div key={ev.id}
                    className="text-[11px] px-1.5 py-0.5 rounded-md truncate font-medium text-white"
                    style={{ backgroundColor: ev.color }}>
                    {ev.type === 'task' ? '● ' : ''}{ev.title}
                  </div>
                ))}
                {more > 0 && (
                  <div className="text-[11px] text-gray-400 px-1.5">+{more} ещё</div>
                )}
              </div>
            </div>
          );
...
function WeekView({ weekStart, events, today, onSelectDay }: {
  weekStart: Date;
  events: CalendarEvent[];
  today: string;
  onSelectDay: (date: string) => void;
  ...
  for (const [date, evs] of eventsByDate) {
    allDayByDate.set(date, evs.filter(e => e.isAllDay));
    timedByDate.set(date, evs.filter((e): e is PersonalEvent => !e.isAllDay && e.type === 'personal'));
  }
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="grid border-b border-gray-100" style={{ gridTemplateColumns: '60px repeat(7,1fr)' }}>
        ...
        {days.map(d => {
          ...
          const isToday = dateStr === today;
          return (
            <div key={dateStr} onClick={() => onSelectDay(dateStr)}
              className="py-3 text-center cursor-pointer hover:bg-gray-50 border-r border-gray-100 last:border-r-0">
              <div className="text-xs font-medium text-gray-500 uppercase">{DAYS_SHORT[getDayIndex(d)]}</div>
              <div className={`w-9 h-9 flex items-center justify-center rounded-full text-base font-semibold mx-auto mt-1
      ...
      <div className="grid border-b border-gray-100" style={{ gridTemplateColumns: '60px repeat(7,1fr)' }}>
        <div className="border-r border-gray-100 flex items-center justify-center">
          <span className="text-[10px] text-gray-400 font-medium">весь<br/>день</span>
        </div>
        {days.map(d => {
          const dateStr = toLocalDateString(d);
          const evs = allDayByDate.get(dateStr) ?? [];
          return (
            <div key={dateStr} className="min-h-[40px] p-1 border-r border-gray-100 last:border-r-0">
              {evs.map(ev => (
                <div key={ev.id} onClick={() => onSelectDay(dateStr)}
                  className="text-[11px] px-1.5 py-0.5 rounded-md truncate font-medium text-white mb-0.5 cursor-pointer"
                  style={{ backgroundColor: ev.color }}>
                  {ev.title}
                </div>
              ))}
      ...
      <div className="overflow-y-auto" style={{ maxHeight: '500px' }}>
        {hours.map(h => (
          <div key={h} className="grid border-b border-gray-50" style={{ gridTemplateColumns: '60px repeat(7,1fr)' }}>
            ...
            {days.map(d => {
              ...
              const evs = (timedByDate.get(dateStr) ?? []).filter(e => {
                const hh = e.startTime ? parseInt(e.startTime.split(':')[0]) : null;
                return hh === h;
              });
              return (
                <div key={dateStr} className="border-r border-gray-100 last:border-r-0 min-h-[48px] p-0.5 relative">
                  {evs.map(ev => (
                    <div key={ev.id} onClick={() => onSelectDay(dateStr)}
...
function DayView({ date, events, today, onAddEvent }: {
  date: string;
  events: CalendarEvent[];
  today: string;
  onAddEvent: () => void;
  ...
  const timed = events.filter((e): e is PersonalEvent => !e.isAllDay && e.type === 'personal');
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="p-5 border-b border-gray-100 flex items-center justify-between">
        <div>
          <div className={`text-3xl font-bold ${isToday ? 'text-[#00CC00]' : 'text-gray-900'}`}>{d.getDate()}</div>
          <div className="text-gray-500 text-sm">{DAYS_FULL[getDayIndex(d)]}, {MONTHS_RU_GEN[d.getMonth()]} {d.getFullYear()}</div>
        </div>
      ...
      {allDay.length > 0 && (
        <div className="px-4 py-3 border-b border-gray-100">
          <div className="text-xs font-medium text-gray-400 mb-2 uppercase tracking-wide">Весь день</div>
          <div className="space-y-1.5">
            {allDay.map(ev => (
              <div key={ev.id} className="flex items-center gap-2 px-3 py-2 rounded-xl text-white text-sm font-medium"
                style={{ backgroundColor: ev.color }}>
                {ev.type === 'task' && <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                </svg>}
                <span>{ev.title}</span>
                {ev.type === 'task' && <span className="ml-auto text-xs opacity-80">{statusLabel(ev.status)}</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="overflow-y-auto" style={{ maxHeight: '500px' }}>
        {hours.map(h => {
          const hEvs = timed.filter(e => e.startTime && parseInt(e.startTime.split(':')[0]) === h);
          return (
            <div key={h} className="flex border-b border-gray-50 min-h-[56px]">
              <div className="w-16 flex-shrink-0 py-2 text-right pr-3 text-xs text-gray-400 font-medium">{h}:00</div>
              <div className="flex-1 py-1 px-2 relative">
                {hEvs.map(ev => (
                  <div key={ev.id} className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-white text-sm font-medium mb-1"
                    style={{ backgroundColor: ev.color }}>
                    <span className="text-xs opacity-80">{ev.startTime}{ev.endTime ? `–${ev.endTime}` : ''}</span>
                    <span>{ev.title}</span>
                  </div>
                ))}
              </div>
...
function CalendarContent({ user }: { user: User }) {
  ...
  const fetchEvents = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/volunteer/calendar?year=${year}&month=${month + 1}`);
      if (!res.ok) return;
      const data = await res.json();
  ...
  function handleDeleteEvent(id: string) {
    fetch(`/api/volunteer/calendar/events/${id}`, { method: 'DELETE' })
      .then(() => {
  ...
  async function handleAddTask(taskId: string, deadline: string, title: string) {
    const res = await fetch('/api/volunteer/calendar/events', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title, startDate: deadline, endDate: deadline,
        color: '#00CC00', isAllDay: true, taskId,
      }),
    });
    if (res.ok) {
      const data = await res.json();
      ...
      const newEv: PersonalEvent = {
        id: data.event.id, type: 'personal',
        title, date: deadline, endDate: deadline,
        color: '#00CC00', isAllDay: true, linkedTaskId: taskId,
      };
      ...
      <div className="min-h-screen bg-green-50 pb-20 lg:pb-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
              ...
              <div className="flex bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                {(['year','month','week','day'] as ViewMode[]).map(v => (
                  <button key={v} onClick={() => setView(v)}
                    ...
                    {v === 'year' ? (t.calendar?.viewYear || 'Год') : v === 'month' ? (t.calendar?.viewMonth || 'Месяц') : v === 'week' ? (t.calendar?.viewWeek || 'Неделя') : (t.calendar?.viewDay || 'День')}
                  </button>
                ))}
              </div>
          ...
          {loading ? (
            ...
            <>
              {view === 'year' && (
                <YearView year={year} events={events} today={today} onSelectDay={handleSelectDay} />
              )}
              {view === 'month' && (
                <MonthView year={year} month={month} events={events} today={today} onSelectDay={handleSelectDay} />
              )}
              {view === 'week' && (
                <WeekView weekStart={weekStart} events={events} today={today} onSelectDay={handleSelectDay} />
              )}
              {view === 'day' && (
                <DayView date={toLocalDateString(currentDate)} events={events.filter(e => e.date === toLocalDateString(currentDate))} today={today} onAddEvent={() => openNewEvent(toLocalDateString(currentDate))} />
              )}
            </>
          )}
          ...
          {!loading && (view === 'month' || view === 'week') && (
            <TaskSidebar events={events} today={today} addedTaskIds={addedTaskIds} onAddTask={handleAddTask} />
          )}
        </div>
      </div>
...
function TaskSidebar({ events, today, addedTaskIds, onAddTask }: {
  events: CalendarEvent[];
  today: string;
  addedTaskIds: Set<string>;
  onAddTask: (taskId: string, deadline: string, title: string) => Promise<void>;
  ...
  const { t } = useTranslation('volunteer');
  const tasks = events.filter((e): e is TaskEvent => e.type === 'task')
    .sort((a, b) => a.date.localeCompare(b.date));

  const upcoming = tasks.filter(t => t.date >= today && !['completed', 'confirmed'].includes(t.status)).slice(0, 8);
            ...
            <div key={t.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-xs text-gray-500">{t.project.title}</span>
                  <span className="text-xs text-gray-300">·</span>
                  <span className={`text-xs font-medium ${daysLeft <= 3 ? 'text-red-500' : daysLeft <= 7 ? 'text-amber-500' : 'text-gray-400'}`}>
              ...
              <button
                ...
                title={isAdded ? (t.calendar?.alreadyAdded || 'Уже в календаре') : (t.calendar?.addToCalendar || 'Добавить в календарь')}
                className={`flex-shrink-0 p-2 rounded-xl transition-colors ${
                  isAdded ? 'text-[#00CC00] bg-green-50 cursor-default' :
                  adding === t.taskId ? 'text-gray-300 cursor-wait' :
